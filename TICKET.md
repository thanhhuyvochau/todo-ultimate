...
- **Status**: Complete — `src/main/config/google-calendar-config.ts` injects the app-owned public OAuth Client ID at build time, so users only sign in and select calendars. `google-calendar-service.ts` implements OAuth2 PKCE with the Google-compatible custom protocol `com.ai-task-planner:/oauth2callback`, safeStorage-backed token persistence/refresh, legacy client/token cleanup with one-time reconnect, a 15-minute rolling sync, and busy-event filtering. `src/main/db/migrations/005_calendar_events.sql` plus `calendar-event-repository.ts` cache the current day and next seven days atomically. Typed `calendar:*` IPC is exposed through preload and `useGoogleCalendarStore`; `GoogleCalendarSettings.tsx` only renders for configured builds and provides connect/sync/calendar-selection controls; `TodayView.tsx` renders immutable calendar blocks and conflict warnings with a Re-plan route. The daily planner includes calendar events as non-negotiable input.
- **Acceptance Criteria**:
